...
namespace GTMPVoice.VoiceClient
{
    internal sealed class VoiceSerializer
    {
        ...
        public VoiceSerializer() : this(0)
        {

        }

        public VoiceSerializer(int maxStringLength)
        {
        ...
        private StructInfo GetKnownType(Type t)
        {
        ...
        public void SerializeKnown(NetDataWriter writer, object o)
        {
            GetKnownType(o.GetType()).Write(writer, o);
        }

        public void DeserializeKnown(NetDataReader reader, object target)
        {
            try
            {
                var info = GetKnownType(target.GetType());
                info.Reference = target;
                info.Read(reader);
            } catch (Exception ex)
            {
                NetUtils.DebugWriteError("Error Deserializing {0}: {1}", target.GetType().ToString(), ex.ToString());
            }
        }
    }
}
